...
import { ArchiveBoxIcon, MegaphoneIcon, ChartBarIcon, Cog6ToothIcon, UserIcon, ArrowLeftStartOnRectangleIcon, BellIcon } from "@heroicons/react/24/outline";
        ...
        <>
            <div className="w-12 bg-[var(--color-bg-secondary)] h-full border-r border-r-[var(--color-primary)] flex flex-col">
                ...
                <div className="mt-auto p-1 flex flex-col gap-1">
                    <MenuButton id="notifications" icon={<BellIcon />} disabled={false} badge={1} label="Notifications" onClick={() => { }} />
...
const MenuButton = ({ id, icon, label, disabled, badge, onClick }: { id: string, icon: React.ReactNode, label: string, disabled?: boolean, badge?: number, onClick: () => void }) => {
    return (
        <button type="button" id={id} onClick={onClick} disabled={disabled} className={`group relative w-10 h-10 flex justify-center items-center text-white rounded-md p-2 transition-colors duration-300 ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[var(--color-bg-primary)] active:bg-[var(--color-bg-primary)]'}`}>
            ...
            {badge && <div className="absolute -top-1 -right-1 bg-red-500 text-white w-5 h-5 flex items-center justify-center rounded-full text-xs">
                {badge}
            </div>}
        </button>
    )
