...
import { FileNoAccessError, FileReadError, FileWriteError } from './errors';
import { VFS } from './vfs';
...
describe('VFS', () => {
  beforeEach(() => {
    vfs = new VFS();
    vfs.use(mockVfs).use(mockVfs2, 100);
  });
  ...
  it('should be able to delete a file', async () => {
    jest.spyOn(mockVfs2, 'delete').mockImplementation(async () => {
      return;
    });
  ...
  it('should be able to handle a read error', async () => {
    jest.spyOn(mockVfs2, 'read').mockImplementation(async (path) => {
  ...
  it('should be able to handle write errors', async () => {
    jest.spyOn(mockVfs, 'write').mockImplementation(async (path) => {
      throw new FileNoAccessError(path);
    });
    jest.spyOn(mockVfs2, 'write').mockImplementation(async (path) => {
      throw new FileNoAccessError(path);
    });
    await expect(
      vfs.write('/error', Buffer.from('ERROR'))
    ).rejects.toThrowError(FileWriteError);
  });

  it('should be able to handle write errors and write to an available vfs', async () => {
    jest.spyOn(mockVfs, 'write').mockImplementation(async () => {
      return;
    });
    jest.spyOn(mockVfs2, 'write').mockImplementation(async (path) => {
      throw new FileNoAccessError(path);
